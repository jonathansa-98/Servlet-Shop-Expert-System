...
public class Pregunta {
	...
	public Pregunta parseFromAnswer(String answer) {
		...
		Pregunta pregunta = this;
		pregunta = pregunta.getChildren().get(index);
	    return pregunta;
	}
	
	@Override
	public String toString() {
		String cad ="Label: "+label+"\t\t-\tQuestion: "+question;
		cad += children==null? "\tanswer\n":"\n";
		if (children!=null) {
			for (Iterator<Pregunta> iterator = children.iterator(); iterator.hasNext();) {
				Pregunta pregunta = (Pregunta) iterator.next();
				cad += "\t"+pregunta.toString();
			}
		}
		return cad;
	}
